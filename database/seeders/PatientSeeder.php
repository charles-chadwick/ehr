...
class PatientSeeder extends Seeder
{
    ...
    public function run() : void
    {
        ...
        $characters = collect(json_decode(file_get_contents(database_path('src/rickandmorty_characters.json')),
            true))->whereNotIn('id', [3,5,9,10,23,29,41,59,108,109,111,121,139,146,149,163,187,196,218,21]);
        $already_in = [];
        $counter = 0;
        ...
        foreach ($characters as $character) {
            ...
            $admin = User::where('role', '!=', 'Administrator')->inRandomOrder()->first();
            ...
            $prefix = match ($character['gender']) {
                'Male' => 'Mr',
                'Female' => fake()->randomElement(['Mrs', 'Ms', '']),
                default => '',
            };
